...
public class AwsS3UriLoader implements ResourceSource
{
	public AwsS3UriLoader ()
	{
		fClient = null;
	...
	{
		return resourceId.startsWith ( "s3://" );
	}
	...
	public InputStream loadResource ( String resourceId ) throws IOException
	{
		try
		{
			...
			try {
				...
				throw new IOException ( "Invalid S3 URI: " + resourceId, e );
			}
			...
			try
			{
				object = fClient.getObject ( GetObjectRequest.builder().bucket(bucket).key(key).build() );
