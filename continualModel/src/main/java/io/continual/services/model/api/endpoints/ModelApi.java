...
public class ModelApi extends ModelApiContextHelper
{
	...
	public void getModelListForAccount ( CHttpRequestContext context, final String acctId ) throws IOException, ModelServiceRequestException
	{
		handleModelRequest ( context, null, null, new ModelApiHandler ()
		{
				...
				throws IOException, JSONException, ModelServiceRequestException
			{
				...
				final List<String> models = ma.getModelsInAccount ( modelApiContext.getModelRequestContext () );
				
				// return the result
				return new JSONObject ()
					.put ( "status", HttpStatusCodes.k200_ok )
					.put ( "models", JsonVisitor.listToArray ( models ) )
					.toString ()
				;
			}
		} );
