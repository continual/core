...
//		Group getAccount ();
	...
	public static void handleWithContextApiAuth ( CHttpRequestContext context, final ApiHandler h )
	{
		handleWithApiAuth ( context, h );
	}

	public static void handleWithContextApiAuth ( CHttpRequestContext context, final ContextApiHandler h )
	{
		handleWithApiAuthAndAccess ( context, new ApiHandler ()
		{

			@Override
			public String handle ( CHttpRequestContext context, HttpServlet servlet, final UserContext uc ) throws IOException, JSONException
			{
//				try
				{
					// associate an account with the user context
					final Identity user = uc.getUser ();
//					final String acctId = user.getUserData ( "acctId" );
//					if ( acctId == null )
//					{
//						log.warn ( "User " + user.getId () + " does not have an account ID established." );
//						sendStatusCodeAndMessage ( context, HttpStatusCodes.k500_internalServerError, "User data is incomplete." );
//						return null;
//					}
//
//					final Group acctGroup = user.getGroup ( acctId );
//					if ( acctGroup == null )
//					{
//						log.warn ( "User " + user.getId () + " reports account ID " + acctId + " but is not a member of that group." );
//						sendStatusCodeAndMessage ( context, HttpStatusCodes.k500_internalServerError, "User data is incomplete." );
//						return null;
//					}

//					final boolean acctAdmin = user.getGroupIds ().contains ( acctId + "-admin" );
					final boolean acctAdmin = true;
					
					h.handle ( context, servlet, new ApiUserContext ()
					{
						@Override
						public UserContext getUser () { return uc; }

//						@Override
//						public Group getAccount () { return acctGroup; }

						@Override
						public boolean isUserAccountAdmin ()
						{
							return acctAdmin;
						}
					} );
...
//				{
//					context.response ().sendError ( HttpStatusCodes.k503_serviceUnavailable, "There's a problem with the authentication system." );
//				}
